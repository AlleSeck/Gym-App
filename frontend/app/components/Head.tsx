...
const Header = () => {
  ...
  const [isLoggedIn, setIsLoggedIn] = useState(false); // User's login state

  useEffect(() => {
    // Check if the user is logged in by checking if a login cookie exists
    const userLoggedIn = Cookies.get('accessToken'); // Check if a cookie named 'accessToken' exists
